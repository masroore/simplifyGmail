...
const trackers = {
  ...
  "AlphaSights.com": "almond.alphasights.com\\/signature-pixel",
  "Amazon.com": [
    "awstrack.me",
  ...
  "CanaryMedia.com": "canarymedia.com\\/e3t\\/Bto",
  ...
  "Cmail20.com": "cmail20.com\\/t",
  "Coda.io": "coda.io\\/logging\\/ping",
  "CodePen.io": "mailer.codepen.io\\/q",
  "ConneQuityMailer.com": "connequitymailer.com\\/open\\/",
  "ConstantContact.com": ["rs6.net\\/on.jsp", "constantcontact.com\\/images\\/p1x1.gif"], // UglyEmail uses .net/on.jsp?
  ...
  "Ftransaction.com": "ftrans\\d{2}.com\\/linktrack",
  ...
  "Is-tracking-pixel-api-prod.appspot.com":
    "is-tracking-pixel-api-prod.appspot.com",
  "KavilBrothers.com": ["kavilbrothers.com\\/opens", "learnvibes.net\\/opens"],
  ...
  "NationalCar.com": "email.nationalcar.com\\/open.aspx",
  ...
  "SendInBlue.com": "sendib\w{2}.com\\/track\\/",
  "Sendgrid.com": [
    "\\/wf\\/open",
    "\\/ss\\/o",
...
/* PLATFORMS I HAVEN'T FOUND THE PATTERN FOR YET
 * vutu.re/solutions/tracking-and-reporting.aspx
 * liveclicker.com/resources
 * delivra.com
 * livehive.com
 * */

const blocklist = new RegExp(Object.values(trackers).flat().join("|"), "i");

// Getting active tab's ID
let tabId = null;
const getTabId = (event, windowTypeEnum = "normal") => {
  console.log("getTabId", event, event?.tabId, windowTypeEnum);
  if (event && event.tabId) {
    tabId = event.tabId;
  } else {
    chrome.tabs.query(
      {
      ...
      (tabs) => {
        if (tabs[0]) {
          tabId = tabs[0].id;
        } else if (windowTypeEnum === "normal") {
          // See if we're running as shortcut in a separate window
          getTabId(null, "popup");
        }
      }
    );
...
getTabId(null, "normal");

// Update tab id when active tab or window is changed
chrome.tabs.onActivated.addListener(getTabId);
chrome.windows.onFocusChanged.addListener(getTabId);

// Handle image load request -- block any that match url patterns for known trackers
const handleRequest = (requestDetails) => {
  let response = {};
  // if (!justChecked) getTabId();

  // If url doesn't contain '#' it's not an image in an email
  let isProxy = requestDetails.url.includes("#");
  let isImage = requestDetails.type === "image";

  let fromGmail;
  if (requestDetails.initiator) {
    // Chromium
    fromGmail = requestDetails.initiator.includes("//mail.google.com");
  } else {
    // Firefox
    fromGmail = requestDetails.originUrl.includes("//mail.google.com");
  }
  // let isAllowed = requestDetails.url.includes("simplify-load-image");

  // && !isAllowed
  if (isImage && isProxy && fromGmail) {
    let url = requestDetails.url.split("#")[1];
    if (blocklist.test(url)) {
      console.log("BLOCK", url, tabId);

      // Notify extension that tracker was blocked
      chrome.tabs.sendMessage(tabId, { action: "tracker_blocked", url: url });

      // Response to Chrome API to actually block the image from loading
